...
- [x] "🔁 Generate Again" button on the "Done" status message of every base generation — re-runs the *whole* last prompt (all `batch_size` images, fresh seed) for the chat in one tap, for quickly building up a bigger batch without retyping the prompt or hunting down a specific image's own Regenerate button. `generation.repeat()` is the batch-returning sibling `regenerate()` is now built on top of (which still keeps only the first image). Backed by a new one-row-per-chat `last_generation` storage table (`Storage.set_last_generation`/`get_last_generation`), updated after every base generation (fresh or "Again"), independent of any single image's `pending_result` row.
...
- [x] `/settings` also covers the default positive/negative prompt prefixes (`TextFieldMeta`, "Default Positive"/"Default Negative") — edit-in-place free text + reset, same override storage as the numeric fields. These two live on `ModelProfile` itself, not `GenerationParams`, so `apply_profile_override` routes them onto the profile directly instead of into `.defaults` (see `PROMPT_OVERRIDE_FIELDS` in `profiles/loader.py`).
- [x] Saved character designs — `/character save <name> | <positive prompt> [| <negative prompt>]` persists a reusable prompt snippet per chat (`storage.py`'s `character` table); `/characters` lists them and activates one via button tap (`active_character` table, one active per chat at a time, same "sticky until changed" pattern as `/model`'s checkpoint selection). While active, the character's positive prompt is prepended to every generation and its negative prompt is appended after the model profile's own negative prefix (`resolve_generation_params`'s new `extra_negative_prompt` param). Scope note: v1 only covers prompt text, not LoRAs — a character wanting a specific LoRA stack still needs it set on the model profile or `/settings`; folding LoRA selection into a character is a natural follow-up if it turns out to be wanted.
